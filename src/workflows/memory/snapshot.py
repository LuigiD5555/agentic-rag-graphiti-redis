...
from src.workflows.memory.types import ChatMemorySnapshot
from src.workflows.memory.core.state import ConversationState
from src.workflows.memory.compression.pareto import compress_messages, format_compressed_summary
from src.workflows.memory.storage.chat_memory_schema import calculate_ttl_timestamp
...
def create_snapshot(
    ...
    for msg in messages:
        if hasattr(msg, 'content') and hasattr(msg, 'role'):
            # It's a Message object (from src.core.contracts)
    ...
    compressed = compress_messages(message_dicts, target_ratio=0.2)
    summary_dense = format_compressed_summary(compressed)

    # 2. Extract key quotes
    key_quotes = extract_key_quotes(message_dicts, max_quotes=5)

    # 3. Extract keywords for BM25
    keywords = extract_keywords(message_dicts, max_keywords=20)

    # 4. Calculate compression ratio
    original_size = sum(len(msg.get("content", "")) for msg in message_dicts)
