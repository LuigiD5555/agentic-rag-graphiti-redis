...
class AdaptiveHybridScanner:
    ...
    def _scan_loop(
        ...
        while not self._stop_event.is_set() and not self._is_frontier_empty(frontier):
            ...
            if self.metrics and time.time() - self.metrics.last_checkpoint_at > 30:
            ...
            if not first_file_emitted and len(files) > 0 and self.metrics:
    ...
    def _process_sequential(
        ...
        dirs_scanned = 0
        max_dirs_per_iteration = 10  # Process multiple directories per iteration
        
        for _ in range(max_dirs_per_iteration):
    ...
    def _scan_directory_worker(
        ...
        cached = self.cache_manager.is_dir_unchanged(dirpath, options_hash)
        if cached:
            # Return cached files and empty subdirs list
            return cached.files, []
        ...
        try:
            ...
            return dir_files, subdirs_to_add
        
        except (OSError, PermissionError):
            return dir_files, subdirs_to_add
    
    def _update_metrics(self, frontier, visited_dirs: Set[str], files: list[str], dirs_scanned: int):
        """Update scan metrics."""
        if not self.metrics:
            return
        
        frontier_size = self._get_frontier_size(frontier)
        self.metrics.frontier_size_current = frontier_size
        self.metrics.frontier_pending_peak = max(
            self.metrics.frontier_pending_peak, frontier_size
        )
        self.metrics.dirs_scanned_total = dirs_scanned
        self.metrics.files_emitted_total = len(files)
        self.metrics.workers_current = len(self.worker_futures) if self.worker_pool else 1
    
    def _checkpoint(self, files: list[str]):
        """Save checkpoint."""
        if not self.metrics or not self.scan_checkpointer:
            return
        
        self.metrics.last_checkpoint_at = time.time()
        self.metrics.checkpoint_commits_count += 1
        
        # Update files in checkpoint
        if files and self.current_run_id:
            self.scan_checkpointer.add_discovered_files(self.current_run_id, files)
    
    def _complete_run(self, files: list[str], dirs_scanned: int):
        """Mark run as completed."""
        if not self.metrics:
            return
        
        self.metrics.completed_at = time.time()
        self.metrics.scan_duration_seconds = self.metrics.completed_at - self.metrics.started_at
        self.metrics.dirs_scanned_total = dirs_scanned
        self.metrics.files_emitted_total = len(files)
        
        if self.scan_checkpointer and self.current_run_id:
            self.scan_checkpointer.complete_run(self.current_run_id, status="completed")
        
        log.info(
            "Scan run %s completed: %d dirs, %d files, %.2f seconds",
            self.metrics.run_id, dirs_scanned, len(files), self.metrics.scan_duration_seconds
        )
