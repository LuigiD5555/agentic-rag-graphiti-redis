...
class RedisErrorMapper(BaseMapper):
    """Maps Redis-related errors."""
    ...
    def can_map(self, exception: Exception, context: Dict[str, Any]) -> bool:
        ...
        redis_patterns = [
            'redis', 'connection pool', 'maxmemory', 'busygroup',
            'connection reset', 'oom command not allowed'
        ]
        
        return any(pattern in exception_str for pattern in redis_patterns) or \
               'redis' in exception_type.lower()
    
    def map(self, exception: Exception, context: Dict[str, Any]) -> Union[ErrorEvent, PressureEvent]:
        stage = context.get('stage', ProcessingStage.IDEMPOTENCY)
        exception_str = str(exception).lower()
        
        # Check for specific Redis error patterns
        if 'maxmemory' in exception_str:
            # Redis out of memory - this is a pressure event that should trigger cleanup
            return PressureEvent(
                pressure_type=PressureType.RAM,
                current=100.0,  # Assume max memory reached
                ...
                metadata={
                    ...
                    'exception_message': str(exception),
                    'timestamp': time.time(),
                    'redis_error': 'maxmemory'
                }
            )
        elif 'busygroup' in exception_str:
            kind = ErrorKind.RATE_LIMIT
            message = f"Redis consumer group busy: {exception}"
            retryable = True
            severity = Severity.MEDIUM
        elif 'connection' in exception_str or 'pool' in exception_str:
            kind = ErrorKind.UPSTREAM_UNAVAILABLE
            message = f"Redis connection error: {exception}"
            retryable = True
            severity = Severity.HIGH
        else:
            kind = ErrorKind.UNKNOWN
            message = f"Redis error: {exception}"
            retryable = True
            severity = Severity.MEDIUM
