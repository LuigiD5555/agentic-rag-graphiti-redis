    ...
    def process(self, file_path: Path) -> Optional[Path]:
        ...
        try:
            ...
            if result.get("success"):
                ...
                log.info("Tool-office converted %s -> %s", file_path.name, output_path.name)
                return output_path
            log.error("Tool-office conversion failed: %s", result.get("error"))
        except requests.RequestException as exc:
            log.warning("Tool-office unavailable: %s", exc)
        return None
