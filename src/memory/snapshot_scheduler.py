...
class SnapshotScheduler:
    ...
    def __init__(self, snapshot_interval: int = 86400):
        ...
        log.info("SnapshotScheduler initialized with interval=%d seconds", snapshot_interval)
    ...
    def should_create_snapshot(self, thread_id: str) -> bool:
        ...
        if last_time is None:
            ...
            log.debug("Thread %s has no snapshot yet, should create", thread_id[:16])
            return True
    ...
    def mark_snapshot_created(self, thread_id: str, timestamp: Optional[float] = None):
        ...
        log.debug("Marked snapshot created for thread %s at %s", thread_id[:16], ts)
    ...
    def cleanup_old_entries(self, max_age: int = 604800):
        ...
        if old_threads:
            log.info("Cleaned up %d old thread tracking entries", len(old_threads))
