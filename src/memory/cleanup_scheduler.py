...
class CleanupScheduler:
    ...
    async def start(self):
        ...
        if self.running:
            log.warning("CleanupScheduler already running")
            return

        self.running = True
        self._task = asyncio.create_task(self._cleanup_loop())
        log.info("CleanupScheduler started")
    ...
    async def _cleanup_loop(self):
        ...
        log.info("Cleanup loop started, will run every %.1f hours", self.cleanup_interval / 3600)
    ...
    async def _run_cleanup(self):
        ...
        try:
            ...
            log.info("Deleted %d expired snapshots from Weaviate", deleted_snapshots)
        except Exception as e:
            log.error("Failed to cleanup expired snapshots: %s", e, exc_info=True)
        ...
        try:
            ...
            log.error("Failed to cleanup old tracking entries: %s", e, exc_info=True)

        elapsed = time.time() - start_time
        log.info("Cleanup tasks completed in %.2f seconds", elapsed)
